...
from gpt4o_completion import chatgpt_completion

from utils.sql import sql_pred_parse, sv_dict_to_string
from prompting_mwoz_correctionlm import get_zeroshot_correction_prompt, conversion, pure_table_prompt
...
def run(test_set, turn=-1, use_gold=False):
    ...
    for data_item in tqdm(selected_set):
        ...
        completion = ""
        if use_gold:
            prompt_text = get_zeroshot_correction_prompt(
        ...
        print(prompt_text.replace(conversion(pure_table_prompt), ""))
        ...
        while not complete_flag:
            ...
            completion = completion.replace("SELECT * FROM", "").strip()
       
            try:
                print("full completion: ", completion)
                #reflection, completion = completion.split("Correct SQL: SELECT * FROM")
                #reflection = reflection.split("Reflection:")[1]
