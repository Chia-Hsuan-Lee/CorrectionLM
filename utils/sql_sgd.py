...
def sql_pred_parse(pred):
    ...
    if " AS " in pred:
        ...
        slot_values_str = str(stmt.tokens[-1]).replace("""'""", "").replace("WHERE ", "")
    ...
    # remove _ in SQL columns # don't need it in sgd
    ...
    #TODO: already removed ";" from last value
    for slot, value in pred_slot_values.items():
        if value[-1] == ";":
            pred_slot_values[slot] = value[:-1]

    return { slot: value for slot, value in pred_slot_values.items() if "-" in slot and value}
...
#pred = "RideSharing_2 WHERE ride_type = 'Regular' AND number_of_seats = 4 ORDER BY ride_fare ASC LIMIT 1"
#pred = "RideSharing_2 WHERE destination = 1008 Blossom Hill Road"
#pred = "Travel_1 WHERE good_for_kids = True"
#pred = "Calendar_1 AS t1, Services_1 AS t2 WHERE t1.event_date = March 4th AND t2.appointment_date = March 4th AND t2.appointment_time = half past 1 in the afternoon"
pred = "RideSharing_2 WHERE number_of_seats = 4 ORDER BY ride_fare ASC LIMIT 1"
pred_slot_values = sql_pred_parse(pred)

#print(pred_slot_values)