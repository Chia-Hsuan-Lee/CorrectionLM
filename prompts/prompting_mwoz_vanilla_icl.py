...
# adapted from ICDST: https://github.com/Yushi-Hu/IC-DST/blob/main/prompting.py
...
def get_icl_prompt(data_item, examples, given_context=None, n_examples=None):
    ...
    prompt_text += f"Based on the previous examples, continue generating a valid SQLite query to answer the following multi-turn conversational question for the given SQL tables. Only output the ramining part of the SQLite query. If the user input does not request any additional information or action, generate WHERE ; as the output.\n"
