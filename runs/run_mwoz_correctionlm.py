# adapated from https://github.com/Yushi-Hu/IC-DST/blob/main/run_codex_experiment.py
import os
import json
import argparse
import copy
...
from prompts.prompting_mwoz_correctionlm import get_correction_prompt, conversion, pure_table_prompt
...
# read the ontology and the test setd
if args.mwz_ver == '2.1':
...
def run(test_set, turn=-1, use_gold=False):
    ...
    for data_item in tqdm(selected_set):
        ...
        completion = "WHERE"
        if use_gold:
            prompt_text, _ = get_correction_prompt(
        ...
        data_item['prompt_text_no_tables'] = prompt_text_no_tables

        print("prompt_text_no_tables: ", prompt_text_no_tables)
        ...
        completion = LM_instance.completion(prompt_text_no_tables)     
        print("full completion: ", completion)
       
        try:
            completion = completion.split("SELECT * FROM")[1]
        except:
            print("completion does not include SELECT * FROM")
            completion = "WHERE"
